...
def Cover():
    ...
    def pwdQuit():
        ...
        if pwdQuit == config["EMERGENCY_KEY"]:
            ...
            config["pwdLastSave"]="5F67UW4357JW54Y89"
            config["isLocking"]=False
            isNowSet=False
            saveConfig(config)
            cover.destroy()
            return
        ...
        def pwdchange():
            ...
            def changepwd1():
                ...
                if not pwdA or not pwdB or not pwdOld:
                    ...
                    return
                if pwdOld != config["pwdLastSave"] and pwdOld != config["EMERGENCY_KEY"]:
...
tk.Button(Center, text="设置", bg='white', fg='black',command=Setting2,).pack(side="left", anchor="sw", padx=10, pady=10)

tk.Button(Center, text="退出", bg='white', fg='black',command=Center.destroy).pack(side="left", anchor="sw", padx=1, pady=10)

if config["isLocking"] == True:
    Cover()
